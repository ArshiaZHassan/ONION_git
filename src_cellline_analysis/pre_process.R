...
#replace NA values with (gene-wise) mean values
k <- which(is.na(Achilles_gene_effect), arr.ind=TRUE)
Achilles_gene_effect[k] <- colMeans(Achilles_gene_effect, na.rm=TRUE)[k[,1]]# replace na with gene average

#save pre-processed DepMap 20q2 gene_effect data
write.table(Achilles_gene_effect,file="../data/depmap_q2_2020_nona_mean_t.tsv",sep='\t',row.names = TRUE,col.names = TRUE, quote=FALSE)
