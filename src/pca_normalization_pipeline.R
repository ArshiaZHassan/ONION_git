...
#PCA-normalization and evaluation: 
#Apply Principal Component Analysis(RPCA) to create normalized data
...
#cols1<-grey.colors(10)
cols1<- brewer.pal(n = 9, "GnBu")[6:9] #pca
...
layer_list_2<-c(1,5,9,19)
for(layer in layer_list)
{
	print(layer)
  #generate normalized and reconstructed data
  v <- pca$rotation[,1:layer]
  projected <- as.matrix(data) %*% v %*% t(v) #PCA-reconstructed data
  norm_data <- data - projected #PCA-normalized data
...
plot_list = c(1,3:length(curve_labels))
generate_corum_similarity_plot(corum_sim_projected, output_folder, paste(file_ext,'projected',sep=''), curve_labels[plot_list], col_set[plot_list], "All gene pairs" ) #[3:length(curve_labels)]
